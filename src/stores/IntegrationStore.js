...
class IntegrationStore {
  ...
  importAgg = (args) => {
    if (
      args.templateType.value === "4" ||
      args.templateType.value === "5" ||
      args.templateType.value === "6"
    ) {
      this.setDataSet(args);
      this.setImportData(true);
    } else {
  ...
  downloadData = (args) => {
    const blob = new Blob([JSON.stringify(args.canBeSaved, null, 2)], {
  ...
  @action downloadProgramData = () => {
    ...
    if (trackedEntityInstancesUpdate.length > 0) {
      const blob = new Blob(
        [
          JSON.stringify(
            { trackedEntityInstances: trackedEntityInstancesUpdate },
            null,
            2
          ),
        ],
    ...
    if (newEnrollments.length > 0) {
      const blob = new Blob(
        [JSON.stringify({ enrollments: newEnrollments }, null, 2)],
        { type: "application/json" }
      );
    ...
    if (newEvents.length > 0) {
      const blob = new Blob([JSON.stringify({ events: newEvents }, null, 2)], {
        type: "application/json",
      });
    ...
    if (newEvents.length > 0) {
      const blob = new Blob(
        [JSON.stringify({ events: eventsUpdate }, null, 2)],
        { type: "application/json" }
      );
  ...
  handleNextAggregate = async () => {
    if (
      ...
      (this.dataSet.templateType.value === "4" ||
        (this.dataSet.templateType.value === "5" &&
          this.dataSet.multiplePeriods)) &&
      this.activeAggregateStep === 4
    ) {
  ...
  allAggregateStepsCompleted() {
    return (
      this.completedAggregate === this.totalAggregateSteps - this.skippedSteps()
    );
  }
  ...
  executeEditIfAllowedAgg = async (model) => {
    ...
    try {
      ...
      const { dataValues } = await api.get(
        "dataSets/" + model.id + "/dataValueSet",
        {}
      );
      ...
      this.closeDialog();
      NotificationManager.error(
        `${e.message} could not fetch data value sets`,
        "Error",
        5000
      );
    }
  ...
  @action deleteSchedule = async (schedule) => {
    ...
    try {
      ...
      NotificationManager.error(
        `Could not save schedule ${e.message}`,
        "Error",
        5000
      );
    }
  ...
  fetchPrograms = async () => {
    ...
    try {
      const {
        programs,
        pager: { total },
      } = await api.get(`programs?${stringParams}`);
      ...
      NotificationManager.error(
        `${e.message} could not fetch programs`,
        "Error",
        5000
      );
      this.closeDialog();
  ...
  fetchDataSets = async () => {
    ...
    try {
      let {
        dataSets,
        pager: { total },
      } = await api.get(`dataSets?${stringParams}`);
      ...
      dataSets = dataSets.map((dataSet) => {
        ...
        const groupedDataElements = _.groupBy(
      ...
      this.closeDialog();
      NotificationManager.error(
        `${e.message} could not fetch data sets`,
        "Error",
        5000
      );
    }
  ...
  get disableNext() {
    if (this.activeStep === 2) {
      if (!this.program.templateType) {
        ...
        return (
          !this.program.data ||
          this.program.data.length === 0 ||
          this.program.mappingName === ""
        );
      } else if (this.program.templateType.value === "2") {
        try {
      ...
      return (
        (!this.program.enrollmentDateColumn &&
          this.program.createNewEnrollments) ||
        (this.program.incidentDateProvided && !this.program.incidentDateColumn)
      );
    } else if (
      this.activeStep === 4 &&
      (this.program.createEntities || this.program.updateEntities)
    ) {
      return !this.program.mandatoryAttributesMapped;
    } else if (this.activeStep === 5) {
      return (
        !this.program.compulsoryDataElements ||
  ...
  get disableNextAggregate() {
    if (this.activeAggregateStep === 2) {
      if (this.dataSet.templateType) {
        if (
          this.dataSet.templateType.value === "1" ||
          this.dataSet.templateType.value === "2" ||
          this.dataSet.templateType.value === "3"
        ) {
          return (
            _.keys(this.dataSet.data).length === 0 ||
            this.dataSet.mappingName === ""
          );
        } else {
          let cond;
          try {
      ...
      if (
        this.dataSet.templateType.value === "1" ||
        this.dataSet.templateType.value === "6"
      ) {
        return (
          _.keys(this.dataSet.data).length === 0 ||
        ...
        return (
          ...
          (this.dataSet.multiplePeriods &&
            (!this.dataSet.startPeriod || !this.dataSet.endPeriod)) ||
        ...
        return (
          ...
          !(
            ...
            (this.dataSet.multiplePeriods &&
              this.dataSet.startPeriod &&
              this.dataSet.endPeriod)
          ) ||
  ...
  get nextLabel() {
    if (this.activeStep === 0) {
      ...
      this.activeStep === 5 &&
      this.program.fetchingEntities === 1 &&
      this.program.isTracker
    ) {
      return "Fetching tracked entity instances";
    } else if (this.activeStep === 6) {
      if (
        this.program.totalImports > 0 &&
        this.program.processed.conflicts.length > 0
      ) {
  ...
  @computed get currentDataSets() {
    ...
    return this.searchedDataSets.slice(
  ...
  @computed get sourceProgramUnits() {
    ...
    return this.program.sourceOrganisationUnits.slice(
      info.page * info.rowsPerPage,
      info.page * info.rowsPerPage + info.rowsPerPage
    );
  }
  ...
  @computed get currentPrograms() {
    ...
    return this.searchedPrograms.slice(
      info.page * info.rowsPerPage,
      info.page * info.rowsPerPage + info.rowsPerPage
    );
  }

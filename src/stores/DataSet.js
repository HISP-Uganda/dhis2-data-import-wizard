...
class DataSet {
  ...
  @action loadLevelsAndDataSets = async () => {
    ...
    if (urlBase) {
      ...
      if (data) {
        const dataSets = data.dataSets.filter(ds => this.periodType === ds.periodType).map(d => {
      ...
      this.dhis2DataSet.dataSetElements.forEach(dse => {
        dse.dataElement.categoryCombo.categoryOptionCombos.forEach(coc => {
          cocs = [...cocs, {
            ...
            value: { dataElement: dse.dataElement.id, categoryOptionCombo: coc.id }
          }]
        });
      });
    }
  ...
  @computed get dhis2DataElements() {
    if (this.dhis2DataSet) {
      const elements = this.dhis2DataSet.dataSetElements.map(e => [e.dataElement.id, e.dataElement.categoryCombo.categoryOptionCombos.map(coc => {

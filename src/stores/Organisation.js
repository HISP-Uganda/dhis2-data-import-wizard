...
class Organisation {
    ...
    @action process = async () => {
        ...
        const openingDate = new Date(2019, 1, 1).toISOString();
        if (this.data && this.mappings.length > 0) {
            ...
            for (const k of keys) {
                ...
                const units = current.data.map(o => {
                    return api.post('organisationUnits', o)
                });
                await Promise.all(units);
                const schemas = current.data.map(o => {
                    return api.post('schemas/organisationUnit', o)
                });

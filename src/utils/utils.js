...
export const allSourceWithMapping = (dataSet) => {
  const ous = dataSet.sourceOrganisationUnits
    .filter((ou) => {
...
export const processOrganisationUnits = (dataSet) => {
  if (dataSet.orgUnitStrategy && dataSet.organisationUnits) {
    return _.fromPairs(
      dataSet.organisationUnits.map((o) => {
        if (
          dataSet.orgUnitStrategy &&
          dataSet.orgUnitStrategy.value === "name"
        ) {
          return [o.name.toLowerCase(), o.id];
        } else if (
          dataSet.orgUnitStrategy &&
          dataSet.orgUnitStrategy.value === "code"
        ) {
          return [o.code, o.id];
        }
        return [o.id, o.id];
  ...
  if (templateType.value !== "2" && templateType.value !== "4") {
            ...
            if (foundData) {
              const groupedData = foundData.map((d) => {
                ...
                return {
                  ...
                  orgUnit: d[orgUnitColumn.value]
                    ? d[orgUnitColumn.value]
                    : null,
                  dataElement: element.id,
                  attributeValue: rowData,
                  categoryOptionCombo: d[categoryOptionComboColumn.value]
                    ? d[categoryOptionComboColumn.value].toLowerCase()
                    : null,
                };
              });
            ...
            _.forOwn(coc.mapping, (mapping, dataElement) => {
              const filtered = data.filter((v) => {
                return (
                  mapping &&
                  mapping.value &&
                  v.categoryOptionCombo === mapping.value.toLowerCase() &&
                  v.dataElement === dataElement
                );
              });
              filtered.forEach((d) => {
                const attribute = findAttributeCombo(
                  categoryCombo,
                  d.attributeValue
                );
                if (d["orgUnit"] && attribute) {
                  const orgUnit = searchSourceOrgUnits(
                    d["orgUnit"],
                    sourceOrganisationUnits
                  );
            ...
            _.forOwn(coc.cell, (mapping, dataElement) => {
              ...
              if (!organisationUnitInExcel) {
                ...
                if (foundOU) {
                  ...
                  errors = [
                    ...errors,
                    { error: `Organisation unit ${ou} not found` },
                  ];
                }
              }
              let found;
          ...
          if (
            ...
            if (validate(elements, v.value.dataElement, value)) {
              ...
              errors = [
                ...errors,
                {
                  error: `Value ${value} on cell ${vCell} not valid expected ${
                    elements[v.value.dataElement]
                  }`,
                },
              ];
            }
    ...
    _.forOwn(cell2, (v) => {
      ...
      const found = data
        .filter((d) => {
          return (
            d.dataElement === v.value.dataElement &&
            d.categoryOptionCombo === v.value.categoryOptionCombo
          );
        })
        .map((f) => {
...
export const validText = (dataType, value, dataSource) => {
  switch (dataType) {
    ...
    case "DATETIME":
    case "TIME":
      return moment(value).isValid() || moment(value, "DD-MM-YYYY").isValid();
    case "UNIT_INTERVAL":
...
export const validateValue = (
  ...
  } else if (!!value && validText(dataType, value, dataSource)) {
...
export const removeDuplicates = (evs, stageEventFilters) => {
  if (
    stageEventFilters &&
    stageEventFilters.elements &&
    stageEventFilters.event
  ) {
    evs = _.uniqBy(evs, (v) => {
    ...
    stageEventFilters &&
    stageEventFilters.elements &&
    stageEventFilters.elements.length > 0
  ) {
    evs = _.uniqBy(evs, (v) => {
      const filteredAndSame = stageEventFilters.elements.map((se) => {
        const foundPrevious = _.filter(v.dataValues, {
    ...
    data.forEach((client) => {
      ...
      currentData.forEach((d) => {
        programStages.forEach((stage) => {
          ...
          let eventDate;
          if (
            stage.eventDateColumn &&
            (stage.createNewEvents || stage.updateEvents)
          ) {
          ...
          if (eventDate && mapped.length > 0) {
            mapped.forEach((e) => {
              ...
              const optionSetValue = e.dataElement.optionSetValue;
              ...
              if (!!value && validatedValue !== null) {
                ...
                conflicts = [
                  ...conflicts,
                  {
                    error:
                      optionsSet === null
                        ? "Invalid value " + value + " for value type " + type
                        : "Invalid value: " +
                          value +
                          ", expected: " +
                          _.map(optionsSet.options, (o) => {
                            return o.code;
                          }).join(","),
                    row: client.client,
        ...
        mappedAttributes.forEach((a) => {
          ...
          const optionSetValue = a.trackedEntityAttribute.optionSetValue;
          const validatedValue = validateValue(
            type,
            value,
            optionSetValue,
        ...
        if (enrollmentDateColumn) {
          let enrollmentDate;
          let incidentDate;
          enrollmentDate = moment(d[enrollmentDateColumn.value], "YYYY-MM-DD");
          ...
          if (
            ...
            if (selectEnrollmentDatesInFuture && selectIncidentDatesInFuture) {
              ...
              if (
                enrollmentDate.isBefore(moment()) &&
                incidentDate.isBefore(moment())
              ) {
                enrollmentDates = [
                  ...enrollmentDates,
                  {
                ...
                conflicts = [
                  ...
                  {
                    error:
                      "both enrollment and incident dates are in the feature which is different from program setup",
                    row: client.client,
                  },
                ];
              }
              ...
              if (enrollmentDate.isBefore(moment())) {
                ...
                conflicts = [
                  ...
                  {
                    error:
                      "enrollment date is in the feature which is different from program setup",
                    row: client.client,
                  },
                ];
              }
              ...
              if (incidentDate.isBefore(moment())) {
                ...
                conflicts = [
                  ...
                  {
                    error:
                      "incident date is in the feature which is different from program setup",
                    row: client.client,
                  },
                ];
              }
      ...
      if (client.previous.length > 1) {
        ...
        client.previous.forEach((p) => {
          ...
          if (updateEntities) {
            const nAttributes = _.differenceWith(
              allAttributes[0],
              p["attributes"],
              (a, b) => {
                return (
            ...
            if (nAttributes.length > 0) {
              const mergedAttributes = _.unionBy(
                allAttributes[0],
                p["attributes"],
                "attribute"
              );
          ...
          if (
            ...
            _.forOwn(groupedEvents, (evs, stage) => {
              ...
              const { repeatable, createNewEvents } = stageInfo;
            ...
            _.forOwn(groupedEvents, (evs, stage) => {
              ...
              const { repeatable, updateEvents, createNewEvents } = stageInfo;
              ...
              if (repeatable) {
                evs.forEach((e) => {
                  ...
                  if (eventIndex !== -1 && updateEvents) {
                    ...
                    const differingElements = _.differenceWith(
                      e["dataValues"],
                      stageEvent["dataValues"],
                      (a, b) => {
                        return (
        ...
        if (orgUnits.length > 1) {
          ...
          if (orgUnit && orgUnit.mapping) {
            ...
            errors = [
              ...errors,
              {
                error: "Organisation unit " + orgUnits[0] + " not found",
                row: client.client,
              },
            ];
          }
...
export const searchSavedEvent = (programStages, event, eventsData) => {
  ...
  const identifiesEvents = programStageDataElements
    .filter((psde) => {
  ...
  const value = _.orderBy(filtered, ["dataElement"], ["asc"])
    .map((dv) => dv.value)
    .join("@");
  ...
  if (eventDateIdentifiesEvent && identifiesEvents.length > 0) {
    ...
    if (currentEvent && updateEvents) {
      ...
      if (ev2 && !many1) {
        const difference = event["dataValues"].filter((x) => {
          const search = ev2["dataValues"].find(
            (v) => v.dataElement === x.dataElement
          );
        ...
        if (difference.length > 0) {
          const updatedDataValues = ev2["dataValues"].map((dv) => {
            const search = difference.find(
              (v) => v.dataElement === dv.dataElement
            );
            if (search) {
          ...
          return {
            ...
            duplicates: false,
            dataValues: [
              ...updatedDataValues.filter((e) => e.value !== ""),
              ...missing,
            ],
          };
        }
    ...
    if (currentEvent && updateEvents) {
      ...
      if (ev1 && !many1) {
        const difference = event["dataValues"].filter((x) => {
          const search = ev1["dataValues"].find(
            (v) => v.dataElement === x.dataElement
          );
          return search && String(search.value) !== String(x.value);
        });
...
export const processEvents = (program, data, eventsData) => {
  ...
  const events = data
    ...
    .filter((e) => e !== null && e.orgUnit !== null)
    .map((ev) => {
      return searchSavedEvent(programStages, ev, eventsData);
    });

  const foundConflicts = events
    .filter((e) => e && e.duplicates === true)
    .map((e) => {
      return {
  ...
  const eventsUpdate = events
    .filter((e) => e && e.update === true && e.duplicates === false)
    .map((e) => {
      return _.omit(e, ["update", "duplicates", "row"]);
    });
  const newEvents = events
    .filter((e) => e && e.update === false && e.duplicates === false)
    .map((e) => {
      return _.omit(e, ["update", "duplicates", "row"]);
    });

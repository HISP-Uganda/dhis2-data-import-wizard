...
class D3 extends React.Component {
  ...
  displayMapping = (de) => {
    if (this.integrationStore.dataSet.templateType.value === '1' || this.integrationStore.dataSet.templateType.value === '6' || this.integrationStore.dataSet.templateType.value === '4') {
      return <TableCell>
        <Select
          ...
          onChange={de.handelMappingChange(this.integrationStore.dataSet.isDhis2 ? this.integrationStore.dataSet.dhis2DataElements : this.integrationStore.dataSet.data, this.integrationStore.dataSet.categoryOptionComboColumn, this.integrationStore.dataSet.isDhis2)}
        />
      </TableCell>
    }
    return null;
  ...
  async componentDidMount() {
    ...
    } else if (this.integrationStore.dataSet.templateType.value === '1' || this.integrationStore.dataSet.templateType.value === '6' || this.integrationStore.dataSet.templateType.value === '4') {
  ...
  render() {
    ...
    if (this.integrationStore.dataSet.templateType.value !== '2' && this.integrationStore.dataSet.templateType.value !== '4') {
      ...
      display = <div className="scrollable">
        <Table>
          <TableHead>
            <TableRow>
              <TableCell style={{ width: '49%' }}>Destination Data Element with disaggregation</TableCell>
              <TableCell style={{ width: '49%' }}>Source Data Element with disaggregation</TableCell>
              <TableCell style={{ width: '2%' }}>Mapped?</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {this.integrationStore.dataSet.allCategoryOptionCombos.map(de => {
              return <TableRow key={de.label} hover>
                <TableCell>
                  {de.label}
                </TableCell>
                <TableCell>
                  {this.displayDynamicCoC(de)}
                </TableCell>

                <TableCell>
                  {!!this.integrationStore.dataSet.cell2[de.label] ? <Done /> : <Clear />}
                </TableCell>
              </TableRow>
            })}
          </TableBody>

...
class D3 extends React.Component {
  ...
  displayMapping = (de) => {
    if (
      ...
      this.integrationStore.dataSet.templateType.value === "6" ||
      ...
      return (
        <TableCell>
          <Select
            ...
            onChange={de.handelMappingChange(
              this.integrationStore.dataSet.isDhis2
                ? this.integrationStore.dataSet.dhis2DataElements
                : this.integrationStore.dataSet.data,
              this.integrationStore.dataSet.categoryOptionComboColumn,
              this.integrationStore.dataSet.isDhis2
            )}
          />
        </TableCell>
  ...
  componentDidMount() {
    if (this.integrationStore.dataSet.templateType.value === "2") {
      ...
      this.integrationStore.dataSet.autoMapDataSet();
  ...
  displayDynamicCoC = (de) => {
    return (
      <Select
        placeholder="Select data element and category option combo"
        isClearable
        isSearchable
        value={this.integrationStore.dataSet.cell2[de.label]}
        options={this.integrationStore.dataSet.allSourceOptionCombos}
        onChange={this.integrationStore.dataSet.setMappingAll3(de)}
        styles={changeStyle(this.integrationStore.dataSet.cell2[de.label])}
      />
    );
  };
  ...
  render() {
    ...
    if (
      ...
      displayMappingHeader = (
        <TableCell style={{ minWidth: 400, padding: 2 }}>Mapping</TableCell>
      );
    }
    ...
    if (
      ...
      display = dataSet.forms.map((form, k) => {
        return (
          <ExpansionPanel
            key={k}
            expanded={this.integrationStore.expanded === k}
            ...
            <ExpansionPanelDetails>
              <div className="scrollable">
                <Table style={{ minWidth: "100%" }}>
                  <TableHead>
                    <TableRow>
                      <TableCell style={{ minWidth: 300, padding: 2 }}>
                        Data Element
                      </TableCell>
                      {displayMappingHeader}
                    ...
                    {form.dataElements.map((de) => {
                      return (
                        ...
                        >
                          ...
                          <TableCell>
                            {form.status[de.id].all ? (
                              <DoneAll />
                            ) : form.status[de.id].some ? (
                              <Done />
                            ) : (
                              <Clear />
                            )}
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell style={{ width: "2%" }}>Mapped?</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {this.integrationStore.dataSet.mergedCellsWithDataElementRow.map(
                (de) => {
                  return (
                    <TableRow key={de.column} hover>
                      <TableCell>
                        {typeof de.name === "object"
                          ? JSON.stringify(de.name)
                          : de.name}
                      </TableCell>
                      <TableCell>{this.displayDynamicCell(de)}</TableCell>

                      <TableCell>
                        {!!this.integrationStore.dataSet.cell2[de.name] ? (
                          <Done />
                        ) : (
                          <Clear />
                        )}
      ...
      display = (
        <div className="scrollable">
          <Table>
            ...
            <TableBody>
              {this.integrationStore.dataSet.allCategoryOptionCombos.map(
                (de) => {
                  return (
                    <TableRow key={de.label} hover>
                      <TableCell>{de.label}</TableCell>
                      <TableCell>{this.displayDynamicCoC(de)}</TableCell>

                      <TableCell>
                        {!!this.integrationStore.dataSet.cell2[de.label] ? (
                          <Done />
                        ) : (
                          <Clear />
                        )}

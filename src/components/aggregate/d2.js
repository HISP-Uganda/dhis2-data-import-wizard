...
class D2 extends React.Component {
    ...
    if (this.integrationStore.dataSet.categoryCombo.categories.length > 0) {
      return (
        <Grid container spacing={8}>
          {this.integrationStore.dataSet.categoryCombo.categories.map(
            (category) => {
              return (
                <Grid
                  key={category.id}
                  ...
                  xs={
                    12 /
                    this.integrationStore.dataSet.categoryCombo.categories
                      .length
                  }
                >
                  ...
                  <Select
                    ...
                    options={
                      this.integrationStore.dataSet.templateType.value === "3"
                        ? this.integrationStore.dataSet.attributeCombosInExcel
                          ? columns
                          : category.options
                        : columns
                    }
  ...
  mapping = () => {
    return (
      <Grid spacing={8} container>
        ...
        <Grid xs={3} item>
          <span style={{ fontWeight: "bold" }}>
            Category option combination column
          </span>
          <Select
            ...
            options={this.integrationStore.dataSet.columns}
            onChange={
              this.integrationStore.dataSet.setCategoryOptionComboColumn
            }
            isClearable
            isSearchable
          />
  ...
  periodColumn = () => {
    ...
    return (
      <div>
        {showPeriod ? (
          this.integrationStore.dataSet.periodInExcel ||
          this.integrationStore.dataSet.templateType.value === "2" ? (
            <div>
              <span style={{ fontWeight: "bold" }}>{label}</span>
              <Select
                placeholder={label}
            ...
            <div>
              <PeriodPicker
                ...
                onPickPeriod={(value) =>
                  this.integrationStore.dataSet.pick(value)
                }
              />
            </div>
          )
        ...
        {showHeader && displayFull ? (
          <div>
            <InputField
              ...
              onChange={(value) =>
                this.integrationStore.dataSet.handelHeaderRowChange(value)
              }
            />
            <InputField
              ...
              onChange={(value) =>
                this.integrationStore.dataSet.handelDataRowStartChange(value)
              }
            />
          </div>
        ) : null}
  ...
  organisationUnitMapping = () => {
    return (
      <div>
        <Grid container spacing={8}>
          <Grid item xs={3}>
            <FormControlLabel
              control={
                <Checkbox
                  checked={this.integrationStore.dataSet.showOnlyUnmappedUnits}
                  onChange={
                    this.integrationStore.dataSet.handleShowOnlyUnmapped
                  }
                />
              }
              label="Show only Unmapped organisations"
            />
          </Grid>
          <Grid item xs={9}>
            <InputField
              ...
              type="text"
              fullWidth
              value={this.integrationStore.dataSet.unitsFilter}
              onChange={(value) => this.integrationStore.dataSet.setUnitsFilter(value)}
            />
          </Grid>
        </Grid>
        <Table>
          ...
          <TableBody>
            {this.integrationStore.sourceUnits.map((u) => (
              <TableRow hover key={u.id + u.name}>
                <TableCell>{u.name}</TableCell>
                <TableCell>
                  <Select
                    placeholder="Select destination unit"
                    value={u.mapping}
                    options={this.integrationStore.dataSet.organisationUnits.map(
                      (ui) => {
        ...
        <TablePagination
          component="div"
          count={this.integrationStore.dataSet.totalLength}
          ...
          <FormControlLabel
            control={
              <Checkbox
                ...
                onChange={
                  this.integrationStore.dataSet.handleAttributeCombosInExcel
                }
  ...
  dataSetPeriod = () => {
    return (
      <Grid container spacing={8}>
        <Grid item xs={3}>
          <Checkbox
            checked={this.integrationStore.dataSet.multiplePeriods}
            onChange={this.integrationStore.dataSet.onCheckMultiplePeriods}
            value="checked"
          />{" "}
          Multiple Periods
        </Grid>
        {this.integrationStore.dataSet.multiplePeriods ? (
          ...
          <Grid item xs={9}>
            {this.integrationStore.dataSet.templateType.value === "4" ? (
              <PeriodPicker
                periodType={this.integrationStore.dataSet.periodType}
                onPickPeriod={(value) =>
                  this.integrationStore.dataSet.replaceParam(
                    createParam({
                      param: "period",
                      value: value,
                    })
              ...
              <PeriodPicker
                periodType={this.integrationStore.dataSet.periodType}
                onPickPeriod={(value) =>
                  this.integrationStore.dataSet.replaceParamByValue(
                    createParam({
                      param: "dimension",
                      value: `pe:${value}`,
                    }),
        ...
        <Grid container spacing={8}>
          <Grid item xs={12}>
            <InputField
              ...
              onChange={(value) =>
                this.integrationStore.dataSet.filterChange(value)
              }
            />
            <GroupEditor
              itemStore={this.integrationStore.dataSet.itemStore}
              assignedItemStore={
                this.integrationStore.dataSet.assignedItemStore
              }
      ...
      <div>
        ...
        <Grid container spacing={8}>
          <Grid
            item
            xs={
              this.integrationStore.dataSet.templateType.value === "4" ? 9 : 12
            }
          >
            {this.integrationStore.dataSet.dhis2DataSet
              ? this.organisationUnitMapping()
              : null}
          </Grid>
          {this.integrationStore.dataSet.templateType.value === "4" &&
          !!this.integrationStore.dataSet.currentLevel ? (
            <Grid item xs={3}>
              <Table style={{ minWidth: "100%" }}>
                <TableHead>
                  <TableRow>
                    <TableCell>
                      <Checkbox
                        checked={this.integrationStore.dataSet.allChecked}
                        onChange={this.integrationStore.dataSet.checkAll}
                      />
                      Organisation
                    </TableCell>
                  </TableRow>
                ...
                <TableBody>
                  {this.integrationStore.remoteUnits.map((ou) => {
                    return (
                      <TableRow key={ou.id} hover>
                        <TableCell>
                          {" "}
                          <Checkbox
                            checked={ou.isSelected}
                            onChange={ou.setSelected}
                          />
                          {ou.name}
                        </TableCell>
                      </TableRow>
              ...
              <TablePagination
                component="div"
                count={this.integrationStore.dataSet.remoteOrganisations.length}
                rowsPerPage={
                  this.integrationStore.paging["remote"]["rowsPerPage"]
                }
                page={this.integrationStore.paging["remote"]["page"]}
                backIconButtonProps={{
                  "aria-label": "Previous Page",
                }}
                nextIconButtonProps={{
                ...
                onChangePage={this.integrationStore.handleChangeElementPage(
                  "remote"
                )}
                onChangeRowsPerPage={this.integrationStore.handleChangeElementRowsPerPage(
                  "remote"
  ...
  form1 = () => {
    const showFixedOptions =
      this.integrationStore.dataSet.templateType.value === "3";
    const showMapping =
      this.integrationStore.dataSet.templateType.value === "1";
    const { displayFull } = this.props;
    return (
      <div>
        {showFixedOptions && displayFull ? (
          <Grid container spacing={8}>
            <Grid item xs={12}>
              {this.fixedOption()}
            </Grid>
  ...
  render() {
    return (
      <div>
        {this.step2Form()}
        <Progress
          open={this.integrationStore.dataSet.dialogOpen}
